    ...
    async def files(self, session, name, url, li):
        try:
            ...
            if not os.path.isfile(f'./{self.title}/{name}-{str(loc)}.jpg'):
                ...
                logger.debug(f"{name} files are already exist {url}")
        except Exception as e:  # Timeout
            logger.error(e)
            await asyncio.sleep(2)
            async with aiohttp.ClientSession() as session:
                await self.files(session, name, url, li)

    def parse(self, html, url):
        # parse(html,['주술회전 197화', 'https://manamoa26.com/bbs/'])
        name = url[0]  # 주술회전 197화
        name = name.split()[4].replace('화', '')  # 197
...
if __name__ == "__main__":
    ...
    logger.info('Epsiode list finished')
    # Run Main
